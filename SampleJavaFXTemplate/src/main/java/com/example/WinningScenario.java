...
public class WinningScenario {
    ...
    private TreeSet<Card> playerCards = new TreeSet<Card>();
    private TreeSet<Card> communityCards = new TreeSet<Card>();

    public WinningScenario(Player p, ArrayList<Card> c ){
        playerCards.addAll(p.getPlayerCards());
    ...
    public String getScenario() {
        ...
        if (flushCards != null) {
            ...
            for (int i = 0; i < flushCards.size(); i++) {
                flushRanks.add((int)flushCards.get(i).getNumericValue());
            }
